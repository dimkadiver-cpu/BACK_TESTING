...
Fase 2 — Sync      (scripts/sync_market_data.py)  [solo se ci sono gap]
  ...
  sorgente (bybit o fixture o altro).
  Output: artifacts/market_data/sync_market_data.json

Fase 3 — Gap Validate  (scripts/gap_validate_market_data.py)  [solo se ci sono gap e Validate mode è GAP ]
...
Fase 4 — Validate full  (scripts/validate_market_data.py)  [solo se cliccato tasto `Valida` o se e stata scielta la nuova dir e `Validate mode` è `GAP` ]
...
| **Download TF ** | Multi-select | Es. `1m`, `5m`, `15m`, `1h` — default `1m`,`15m`,`1h`,`4h`,`1D` |
| **Price basis** | Select | `last` (prezzo last standard) / `mark` (mark price, rilevante per futures) | // usate per scaricare il tipo di prezzo
| **Market source** | Select | `bybit` (API Bybit live) / altri provide (per ora in pregetazione) |
| **Validate mode** | Select | `GAP` (esegue 3 le fasi) / `OFF` (salta la fase 3/4 validate full) |
...
| "Market data pronti, gap validati" | Fase 4 completata, gap erano presenti ma ora validati |
| "Market data pronti ma non validati in questa run" | OFF mode completato |
...
| **Price basis** | Select | `last` (prezzo last standard) / `mark` (mark price, rilevante per futures) | // usate per il backtesting da dataset presente
