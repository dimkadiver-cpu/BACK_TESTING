...
- lanciare l’acquisizione o la rilettura
- salvare l’output nel formato previsto dal sistema

Se l’ingestione o parte di essa esiste già nel progetto, deve esserci una verifica funzionale esplicita di:
...
### 4.16 Requisiti di parser management
...
Le modifiche semplici al parser devono essere possibili senza rifattorizzare il core applicativo. Questo requisito non implica che l’intero parser sia editabile via GUI nel MVP, ma impone un design che favorisca riuso e modifica rapida.

### 4.17 Requisiti dataset e chain builder
...
#### Integrazione minima GUI / launcher
Nel bootstrap iniziale la repository deve prevedere anche un punto di ingresso per la GUI operativa o launcher, purché separato dal core simulativo. Una possibile estensione minima è:
...
Regola:
- la GUI orchestri configurazione e lancio
- gli artifact analitici restino nel backend / filesystem
- il core simulativo non dipenda dalla GUI
...
### 11.7 Matrice ufficiale delle variabili configurabili di policy
...
#### Variabili MVP obbligatorie o fortemente consigliate
...
#### Variabili esplicitamente rinviate
...
#### Governance del search space iniziale optimizer
Lo search space iniziale dell’optimizer deve restare ristretto. Parametri iniziali consentiti:
...
#### Nota sul leverage
Il parametro `leverage` non deve essere trattato come variabile centrale della policy MVP né come parametro dello search space iniziale dell’optimizer. Nel MVP può essere ignorato oppure mantenuto solo come metadato o parametro opzionale del blocco `risk`.

### 11.8 Semantica estesa degli ordini pending
Il blocco `pending` della policy governa il comportamento automatico del motore sugli ordini pendenti, mentre il blocco `updates` governa l’applicazione degli update dichiarati dal trader.

Il sistema deve supportare almeno:
- timeout configurabile dei pending
- cancellazione automatica dei pending su timeout
- cancellazione dei pending non fillati se il mercato ha già raggiunto TP1 prima del fill
- cancellazione degli averaging pending residui dopo TP1
- distinzione tra `CANCEL_PENDING` trader-driven e cancellazioni automatiche engine-driven

Regole minime:
- le cancellazioni automatiche devono essere auditabili in `event_log`
- la richiesta trader e l’azione realmente eseguita devono restare distinguibili almeno per `source`
- il comportamento deve essere policy-driven e non hardcoded
...
## 15. Output, packaging e acceptance criteria integrati

### 15.1 Output e artifact
...
Gli artifact costituiscono la fonte di verità analitica del sistema. La GUI può limitarsi a mostrare stato del job, esito sintetico, warning minimi e percorso output.

### 15.2 Packaging, installazione e deploy
...
Anche se il packaging finale non è parte del bootstrap tecnico immediato, il progetto deve considerare fin da subito:
- build distribuibile
- configurazione ambiente separabile
- percorsi e dipendenze gestibili
- possibilità futura di attivazione o licenza

### 15.3 Acceptance criteria integrati
Il sistema soddisfa i requisiti di prodotto se:

**Acquisizione**
- consente di configurare e lanciare la lettura o acquisizione da almeno una delle sorgenti previste
- l’architettura supporta esplicitamente canale Telegram, topic Telegram, chat e DB/dataset esistente

**Parser**
- consente il riuso di parser esistenti e la modifica rapida di parser/vocabolario senza modifiche invasive al core

**Chain building**
- distingue almeno chain complete, signal-only, simulabile/non simulabile, trader assegnato/ambiguo

**GUI**
- fornisce una interfaccia operativa che consente di configurare e lanciare le funzioni senza imporre viste analitiche avanzate

**Policy**
- supporta almeno il set minimo di variabili definito in questo documento
- le regole pending sono controllate da policy e non hardcoded

**Pending**
- gestisce almeno timeout pending, cancellazione su timeout, cancellazione pending non fillato dopo TP1, cancellazione averaging pending dopo TP1
- distingue `CANCEL_PENDING` trader-driven da cancellation engine-driven

**Output**
- genera artifact strutturati separati dalla GUI e utilizzabili per audit o analisi successive

**Deploy**
- è predisposto a packaging e installazione futura fuori dall’ambiente di sviluppo
